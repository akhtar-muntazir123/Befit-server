...
export const getWorkoutsByDate = async (req, res, next) => {
  try {
    ...
    console.log("backend :todays workout",todaysWorkouts)
  ...
  try {
    ...
    for (const line of eachworkout) {
      ...
      if (line.startsWith("#")) {
        ...
        if (existingWorkout) {
          console.log(`Duplicate workout found: ${workoutDetails.name}`);
          return res.status(400).json({
            message: `Workout "${workoutDetails.name}" already exists in the "${workoutDetails.category}" category.`,
          });
        }
    ...
    // Insert valid workouts into the database
